...
def test_file_returns_a_datetime() -> None:
    with tempfile.NamedTemporaryFile() as f:
        result = get_effective_date(Path(f.name), is_directory=False, ignore_patterns=[], file_limit=10000)
        assert isinstance(result, datetime)


def test_empty_directory_falls_back_to_dir_mtime() -> None:
    with tempfile.TemporaryDirectory() as d:
        result = get_effective_date(Path(d), is_directory=True, ignore_patterns=[], file_limit=10000)
        assert isinstance(result, datetime)
...
def test_directory_date_reflects_newest_child() -> None:
    with tempfile.TemporaryDirectory() as d:
        ...
        result = get_effective_date(path, is_directory=True, ignore_patterns=[], file_limit=10000)
        assert result.timestamp() >= new_ts


def test_ignore_pattern_excluded_from_date_scan() -> None:
    import time

    with tempfile.TemporaryDirectory() as d:
        path = Path(d)
        node_modules = path / "node_modules" / "pkg"
        node_modules.mkdir(parents=True)
        hot_file = node_modules / "index.js"
        ...
        result = get_effective_date(
            path, is_directory=True, ignore_patterns=["node_modules"], file_limit=10000
        )
...
def test_file_limit_logs_warning(caplog: pytest.LogCaptureFixture) -> None:
    with tempfile.TemporaryDirectory() as d:
        ...
        with caplog.at_level(logging.WARNING, logger="keep_it_tidy.date.item_date"):
            get_effective_date(path, is_directory=True, ignore_patterns=[], file_limit=3)
